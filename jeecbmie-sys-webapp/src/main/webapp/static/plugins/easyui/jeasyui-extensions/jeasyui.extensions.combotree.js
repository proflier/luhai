/**
 * jQuery EasyUI 1.3.6 Copyright (c) 2009-2014 www.jeasyui.com. All rights
 * reserved.
 * 
 ...
 * 
 * jQuery EasyUI combotree Extensions 1.0 beta jQuery EasyUI combotree 组件扩展
 * jeasyui.extensions.combotree.js 二次开发 流云 最近更新：2014-06-05
 * 
 * 依赖项： 1、jquery.jdirk.js v1.0 beta late 2、jeasyui.extensions.js v1.0 beta late
 * 3、jeasyui.extensions.menu.js v1.0 beta late 3、jeasyui.extensions.combo.js
 * v1.0 beta late 5、jeasyui.extensions.tree.js v1.0 beta late
 * 
...
(function($, undefined) {
	...
	var defaults = $.fn.combotree.extensions.defaults = $.extend({}, $.fn.tree.extensions.defaults, 
		{
			// 更改继承于 easyui-tree 的自定义属性 toggleOnClick 的默认值，使得
			// easyui-combotree 中 tree 组件的页节点在点击后不自动展开/折叠子节点；
			// Boolean 类型，默认为 false。
			toggleOnClick : false,

			// 更改继承于 easyui-tree 的自定义属性 autoBindDblClick 的默认值，使得
			// easyui-combotree 中 tree 组件的页节点在双击后不触发第一个右键菜单项的事件；
			// Boolean 类型，默认为 false。
			...
			onHidePanel:function(){
				...
				for (var i = 0; i < nodes.length; i++) {
					if (nodes[i].text == q) {
						result = false;
						$(this).combotree('setValue', nodes[i].id);
						//t.tree('select', nodes[i].target);
					}
				}
				if(result){
		    		$(this).combo("clear");
