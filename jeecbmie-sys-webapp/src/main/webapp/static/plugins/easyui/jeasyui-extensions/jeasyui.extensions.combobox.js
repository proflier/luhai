...
(function ($, undefined) {
    ...
    var defaults = $.fn.combobox.extensions.defaults = {
        ...
        onHidePanel:function(){
	    	...
	    	var value = $(this).combobox("getValue"); //当前combobox的组件值
	    	var text = $(this).combobox("getText"); //当前combobox的输入值
	    	var allData = $(this).combobox("getData"); //获取combobox所有数据
	    	var result = true; //为true说明输入的值在下拉框数据中不存在
	    	for(var i = 0; i < allData.length; i++){
	    		if(text.toLowerCase() == allData[i][textField].toLowerCase()){
	    			result = false;
	    			$(this).combobox("clear"); //先清除再赋值，不然不会触发onSelect事件
