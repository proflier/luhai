...
(function(jQuery, window, undefined) {
	...
	function migrateWarnProp(obj, prop, value, msg) {
		if (Object.defineProperty) {
			...
			// allow property to be overwritten in case some other plugin wants
			// it
			try {
		...
		if (selector && typeof selector === "string"
			...
			if (jQuery.parseHTML) {
				return oldInit.call(this, jQuery.parseHTML(jQuery
						.trim(selector), context, true), context, rootjQuery);
			}
		}
			...
			if (fragment) {
				...
				handleScript = function(elem) {
					...
					if (!elem.type || rscriptType.test(elem.type)) {
						// Detach the script and store it in the scripts array
						// (if provided) or the fragment
						// Return truthy to indicate that it has been handled
						return scripts ? scripts
								.push(elem.parentNode ? elem.parentNode
										.removeChild(elem) : elem) : fragment
								.appendChild(elem);
					}
				};
				...
				for (i = 0; (elem = ret[i]) != null; i++) {
					...
					if (!(jQuery.nodeName(elem, "script") && handleScript(elem))) {
						...
						if (typeof elem.getElementsByTagName !== "undefined") {
							// handleScript alters the DOM, so use jQuery.merge
							// to ensure snapshot iteration
