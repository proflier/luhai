...
<!-- 			<p style="text-align:center"><button type="button" id="upload">自定义上传按钮</button></p> -->
		...
		function fullAvatarEditor() {
			...
			var file			= '${ctx}/static/plugins/fullAvatar/fullAvatarEditor.swf';		//flash文件的路径
			var	version			= "10.1.0";						//播放该flash所需的最低版本
			var	expressInstall	= '${ctx}/static/plugins/fullAvatar/expressInstall.swf';			//expressInstall.swf的路径
            ...
            swfobject.addDomLoadEvent(function () {
                var swf = new fullAvatarEditor("swfContainer", {
					    id: 'swf',
						upload_url: '${ctx}/accessory/avatarUpload/${currentPid}',
						src_upload:2,
...
// 		                browse_box_width: "50",//图片选择框的宽度
// 		                browse_box_height: "50",//图片选择框的高度
		                avatar_sizes: "100*100",//100*100|50*50|32*32,表示一组或多组头像的尺寸。其间用"|"号分隔。
					}, function callback (msg){
						switch(msg.code)
						{
// 							case 1 : $.messager.alert('提示','页面成功加载了组件！','info');break;
// 							case 2 : alert("已成功加载默认指定的图片到编辑面板。");break;
							case 3 :
								if(msg.type == 0)
								{
									$.messager.alert('提示','摄像头已准备就绪且用户已允许使用','info');
								}
								else if(msg.type == 1)
								{
									$.messager.alert('提示','摄像头已准备就绪但用户未允许使用！','info');
								}
								else
								{
									$.messager.alert('提示','摄像头被占用！','info');
								}
							break;
							case 5 : 
								if(msg.type == 0)
...
// 									alert(msg.content.sourceUrl)
									$("#imgAra").attr("src", "${ctx}/accessory/avatarView/"+msg.content.sourceUrl);  
									dlg_img.panel('close');
								}
							break;
						}
