...
<body>
	<input type="hidden" name="id" value="${customerPerssion.id }" />
	<div>
		<div>
 ...
 var group_setting = {
	...
	var users_show={
		 ...
		 addTreeNode: function(treeNode){
       	...
       	changekeyFlag : function(userId){
       		...
       		for(var i=0,j=this.userPs.length;i<j;i++){
  				...
  				var $curSpan = $("#"+this.userPs[i].userId).find('a>span:first');
  				if(curkeyFlag=='0'){//0
  					if(userId==this.userPs[i].userId){
  						this.userPs[i].keyFlag='1';
  						$curSpan.removeClass("nokeyperson");
  		   				$curSpan.addClass("keyperson");
  					}
  				}else{//1
  					this.userPs[i].keyFlag='0';
		...
		saveSelect : function(row){
			...
			$.ajax({
				async : false,
				url : '${ctx}/system/customerPerssion/saveUserRealtion/'+row.customerCode,
				type : "POST",
				data:{
					"themeMemberIds":	themeMemberIds,
					"themeMemberKeyIds":themeMemberKeyIds
				},
				success : function(data) {}
				});
		}
