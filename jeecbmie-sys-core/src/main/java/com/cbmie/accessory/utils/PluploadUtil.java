package com.cbmie.accessory.utils;
 ...
 * 所以这里更多的是处理普通文件的批量上传。 这里主要处理文件上传（方法注释部分为大文件分段上传预留设计）
 */
public class PluploadUtil {
	 ...
	 * @throws IOException
	 */
	public static Accessory upload(Plupload plupload, File dir) throws IllegalStateException, IOException {
		Accessory accessory = new Accessory();
		// 生成唯一的文件名
		// String filename = "" + System.currentTimeMillis() +
		// plupload.getName();
		String filename = "" + System.currentTimeMillis();
		accessory = upload(plupload, dir, filename);
		return accessory;
	}

	/**
	 * 用于Plupload插件的文件上传（方法注释部分为大文件分段上传预留设计）
	 * 
	 * @param plupload
	 *            - 存放上传所需参数的bean
	 * @param dir
	 ...
	 * @throws IOException
	 */
	public static Accessory upload(Plupload plupload, File dir, String filename)
	...
	/*	MultipartResolver resolver = new CommonsMultipartResolver(
				plupload.getRequest().getSession().getServletContext());
		MultipartHttpServletRequest multipartRequest = resolver.resolveMultipart(plupload.getRequest());
		...
		if (map != null) {
			...
			while (iter.hasNext()) {
				...
				for (MultipartFile multipartFile : fileList) {
					...
					// saveUploadFile(multipartFile.getInputStream(), tempFile,
					// chunk == 0 ? false : true);
					//
					...
					accessory = accessoryInit(multipartFile, dir, filename);
					// }
				}
			}
		}
	...
	public static Accessory accessoryInit(MultipartFile multipartFile, File dir, String fileName) {
		Accessory accessory = new Accessory();
		String realName = multipartFile.getOriginalFilename();
		String accExtension = realName.split("\\.")[1];
		accessory.setAccSrc(dir.getPath());
		accessory.setAccId(Long.parseLong(fileName));
		accessory.setAccRealName(realName);
		accessory.setAccExtension(accExtension);
		return accessory;
	}
