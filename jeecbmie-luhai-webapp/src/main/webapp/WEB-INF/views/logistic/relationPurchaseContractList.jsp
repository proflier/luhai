...
<script type="text/javascript">
    ...
    columns:[[
            ...
            {field:'purchaseContractNo',title:'合同号',sortable:true,width:20},
            {field:'contractCategoryView',title:'合同类别',sortable:true,width:10},
			{field:'deliveryUnitView',title:'供货单位',sort:true,width:20},
			{field:'contractAmount',title:'合同金额',sortable:true,width:20}
    ]],
	});
	
//保存 
function saveForShipTrace(){
	var row_selects = purchase_dg.datagrid("getSelections");
	if(row_selects!=null){
		var returnValue = "";
		for (var i = 0; i < row_selects.length; i++) {
			if(returnValue!=""){
				returnValue = returnValue+","+row_selects[i].innerContractNo;
			}else{
				returnValue = row_selects[i].innerContractNo;
			}
		}
		$("#innerContractNo").val(returnValue);
	}
}
