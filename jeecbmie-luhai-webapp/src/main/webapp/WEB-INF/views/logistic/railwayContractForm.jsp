...
<body>
	<div>
		<form id="mainform"  action="${ctx}/logistic/railwayContract/${action}" method="post">
			...
			<div data-options="title: '合同信息', iconCls: false, refreshable: false" >	
				...
				<table width="98%" class="tableClass">
					<tr>
						...
						<td>
							...
							<input id="innerContractNo" name="innerContractNo" readonly="readonly" type="text" value="${railwayContract.innerContractNo }" class="easyui-validatebox"
							data-options="required:true"/>
						</td>
						<th>合同编号</th>
						<td>
							<input id="contractNo" name="contractNo" type="text" value="${railwayContract.contractNo }" class="easyui-validatebox"
							data-options="required:true"/>
						</td>
					</tr>
					<tr>
						<th>申请部门</th>
						<td>
							<input id="applyDeptId" class="easyui-combotree" name="applyDeptId" type="text" value="${railwayContract.applyDeptId }"/>
						</td>
						<th>申请人</th>
						<td>
							<input id="applyUserId" class="easyui-combobox" name="applyUserId" type="text" value="${railwayContract.applyUserId }"/>
						</td>
					</tr>
					<tr>
						<th>业务经办人</th>
						<td>
							<mytag:combotree name="businessManager" value="${railwayContract.businessManager}"  required="true"  type="userTree" />
						</td>
						<th>申请日期</th>
						<td>
							<input id="applyDate" name="applyDate" type="text" value="<fmt:formatDate value='${railwayContract.applyDate}' />" 
					...
					<tr>
						<th>合同金额</th>
						<td>
							<input id="money" name="money" type="text" value="${railwayContract.money }" class="easyui-numberbox"
							data-options="precision:'2',required:true"/>
						</td>
						<th>币种</th>
						<td>
							<mytag:combobox name="moneyCurrencyCode" value="${railwayContract.moneyCurrencyCode}" type="dict" parameters="currency"/>
						</td>
					</tr>
					<tr>
						<th>合同开始日期</th>
						<td>
							<input id="startDate" name="startDate" type="text" value="<fmt:formatDate value='${railwayContract.startDate}' />" 
							class="easyui-validatebox" data-options="required:true"/>
						</td>
						<th>合同结束日期</th>
						<td>
							<input id="endDate" name="endDate" type="text" value="<fmt:formatDate value='${railwayContract.endDate}' />" 
							class="easyui-validatebox" data-options="required:true"/>
						</td>
					</tr>
					<tr>
						<th>对方单位全称</th>
						<td>
							<mytag:combobox name="traderName" value="${railwayContract.traderName}" type="customer" parameters="10230003" hightAuto="false"/>
						</td>
						<th>对方联系方式</th>
						<td>
							<input id="traderContact" name="traderContact" type="text" value="${railwayContract.traderContact }" class="easyui-validatebox"
							data-options="required:true"/>
						</td>
					</tr>
					<tr>
						<th>公章</th>
						<td>
							<input id="signetCode" name="signetCode" type="text" value="${railwayContract.signetCode }" class="easyui-combogrid"  />
						</td>
						<th  >印章管理员</th>
						<td>
							<input id="sealManagerShow" type="text" readonly="readonly"/>
							<input id="sealManager" name="sealManager" type="hidden" value="${railwayContract.sealManager }"/>
						</td>
					</tr>
					<tr>
						<th>贸易类型</th>
						<td colspan="3">
							<mytag:combobox name="tradeCategory" value="${railwayContract.tradeCategory}" type="dict" parameters="orderShipType"/>
						</td>
					</tr>
					<tr>
						<th>主要内容</th>
						<td colspan="3">
							<textarea rows="3" cols="80" id="content" class="easyui-validatebox" name="content">${railwayContract.content }</textarea>
						</td>
					</tr>
					<tr>
						<th>是否法人签署 </th>
						<td>
							<input id="isCorporationSign" name="isCorporationSign" type="text" value="${railwayContract.isCorporationSign }" class="easyui-combobox"
							data-options="valueField: 'label',textField: 'value',panelHeight:'auto',required:true, data: [{label: '1',value: '是'}, {label: '0',value: '否'}]"/>
						</td>
						<th>代理人</th>
						<td>
							<input id="agent" name="agent" type="text" value="${railwayContract.agent }" class="easyui-validatebox"/>
						</td>
					</tr>
					<tr>
						<th>审查类别 </th>
						<td>
							<mytag:combobox name="checkTypeCode" value="${railwayContract.checkTypeCode}" type="dict" parameters="checkType"/>
						</td>
						<th>是否法务审查</th>
						<td>
							<input id="isLegalReview" name="isLegalReview" type="text" value="${railwayContract.isLegalReview }" class="easyui-combobox"
							data-options="valueField: 'label',textField: 'value',panelHeight:'auto',required:true, data: [{label: '1',value: '是'}, {label: '0',value: '否'}]"/>
						</td>
					</tr>
					<tr>
						<th>重大非常规披露</th>
						<td colspan="3">
							<textarea rows="3" cols="80" id="tipContent" class="easyui-validatebox" name="tipContent">${railwayContract.tipContent }</textarea>
						</td>
					</tr>
					<tr>
						<th  >帐套单位</th>
						<td colspan="3">
							<input id="setUnit" name="setUnit"   type="text" value="${railwayContract.setUnit }"  class="easyui-validatebox"  />
						</td>
					</tr>
				</table>
				</fieldset>
				...
				<table width="98%" class="tableClass">
					<tr>
						...
						<th  >制单部门</th>
						<td>${railwayContract.createrDept }</td>
						<th  >制单时间</th>
						<td>
				...
				<c:if test="${railwayContract.pid ne null }">
					...
					<legend>变更原因</legend>
						<div >
							<input id="changeReason" style="width:45%;height:40px;" name="changeReason" value="${railwayContract.changeReason }" class="easyui-validatebox"/>
						</div>
					</fieldset>
				</c:if>
				<fieldset class="fieldsetClass" >
...
var allCode;
	$(function() {
		...
		if('${railwayContract.sealManager }'!=null && '${railwayContract.sealManager }'!=''){
			$.ajax({
				url:'${ctx}/baseInfo/baseUtil/userNameShow?userId=${railwayContract.sealManager }',
				type:'get',
				success:function(data){
