...
$(function(){   
	dg=$('#dg').datagrid({  
		method: "get",
	    url:'${ctx}/logistic/railwayContract/json?filter_NES_changeState=0', 
	    fit : true,
		fitColumns : true,
		border : false,
		striped:true,
...
//	 	panelHeight : 'auto',
	...
	$("#export_PDF").on("click", function() {
		...
		var url = "${ctx}/logistic/railwayContract/exportPDF/" + row.innerContractNo;
...
function add() {
	d=$("#dlg").dialog({   
	    title: '新增合同',    
	    fit:true,    
	    href:'${ctx}/logistic/railwayContract/create',
	    maximizable:false,
	    modal:true,
	    buttons:[{
...
function del(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/logistic/railwayContract/delete/"+row.id,
				success: function(data){
...
function upd(){
	...
	d=$("#dlg").dialog({   
	    title: '修改合同',    
	    fit:true,    
	    href:'${ctx}/logistic/railwayContract/update/'+row.id,
	    maximizable:false,
	    modal:true,
	    buttons:[{
			text:'提交申请', iconCls:'icon-hamburg-up',
...
function detail(){
	...
	d=$("#dlg").dialog({   
	    title: '合同明细',    
	    fit:true,  
	    href:'${ctx}/logistic/railwayContract/detail/'+row.id,
	    maximizable:false,
	    modal:true,
	    buttons:[{
			text:'关闭',iconCls:'icon-cancel',
...
//申请
function apply(){
	business_page.procStartUrl="${ctx}/logistic/railwayContract/apply";
...
function callBack(){
	...
	parent.$.messager.confirm('提示', '您确定要撤回申请？', function(data){
		if (data){
			...
			$.ajax({
				type:'get',
				url:"${ctx}/logistic/railwayContract/callBack/" + row.id + "/" + row.processInstanceId,
				success: function(data){
			    	successTip(data,dg);
					if(data=='success'){
	...
	parent.$.messager.confirm('提示', '是否确认变更？', function(data){
		if (data){
			d=$("#dlg").dialog({   
			    ...
			    closable:false,
			    style:{borderWidth:0},
			    href:'${ctx}/logistic/railwayContract/change/'+row.id,
			    modal:true,
			    buttons:[
		            {
					text:'提交申请', iconCls:'icon-hamburg-up',
	...
	var selectUsers_dg=$("#dlg_selectUsers").dialog({   
	    ...
	    buttons:[{
			...
			handler:function(){
				...
				$.ajax({
					async : false,
					url : '${ctx}/permission/businessPerssion/updatePermission4RailwayContract/'+row.id,
					type : "get",
					data:{
