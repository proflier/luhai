...
$(function(){
	dg=$('#dg').datagrid({
		...
		columns:[[
		    ...
		    {field:'mortgageInterestO',title:'押汇利息（外币）',align:'right',sortable:true,width:20,
...
//							value = Math.abs(value); //取绝对值
			...
			{field:'mortgageInterestRMB',title:'押汇利息（人民币）',align:'right',sortable:true,width:20,
		    	formatter: function(value,row,index){
					if(isNaN(value)||value==null){
						return 0.00; //如果不是数字，返回0.000
					}else{
//							value = Math.abs(value); //取绝对值
			...
			{field:'allMoney',title:'本息合计',sortable:true,width:20,align:'right',
				formatter: function(value,row,index){
					if(isNaN(value)||value==null){
						return 0.00; //如果不是数字，返回0.000
					}else{
