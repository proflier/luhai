...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function show(themeId) {
				//设置提交地址
				var url = "http://${ip}:${post}${ctx}/mobile/feedback/detail";
				//设置提交方式
				var method = "POST";
				//设置发送数据
			...
			function initList() {
            	//设置提交地址
            	var url = "http://${ip}:${post}${ctx}/mobile/feedback/initList";
            	//设置提交方式
            	var method = "POST";
            	//设置发送数据
            	var data = "page=" + page + "&rows=" + rows + "&title="+ $("filterTitle").value;
            	//设置自定义http头 json格式
            	var requestHeader = {};
            	//设置进度条显示
            	var isShowProgress = false;
	...
	<header>
		<titlebar title="反馈主题" iconhref="window.close();" hidericon="true"/>
		<input type="text" id="filterTitle" prompt="请输入主题名" hideborder="false" style="width:75%;"/>
	...
	<body onload="initList();" backbind="window.close();" onl2rside="window.close();">
		<list id="themeList" onscrollbottom="initList();">
			<item data-role="listheader">
				<hr/>
			</item>
			<item data-role="listitem" style="background-click-color:#e5e5e5">
				<br size="8"/>
				<font id="title" style="font-weight:bold;">主题</font>
				<br size="4"/>
				<font id="detail" style="color:#5f5f5f;font-size:16;">创建时间</font>
				<br size="8"/>
				<hr/>
			</item>
