...
<html id="listWindow">
	<head>
		...
		<script type="text/javascript">
			...
			function show(id) {
				//设置提交地址
				var url = "http://${ip}:${post}${ctx}/mobile/delegate/todo/" + id;
				//设置提交方式
				var method = "GET";
				//设置发送数据
				var data = "";
			...
			function initList() {
            	//设置提交地址
            	var url = "http://${ip}:${post}${ctx}/mobile/delegate/initList";
            	//设置提交方式
            	var method = "POST";
            	//设置发送数据
            	var data = "page=" + page + "&rows=" + rows + "&filter_LIKES_procDefKey="+ $("filterProcDefKey").value;
            	//设置自定义http头 json格式
            	var requestHeader = {};
            	//设置进度条显示
            	var isShowProgress = false;
            	...
            	if (response.status == 200) {
            		...
            		for (i = 0; i < data.length; i++) {
				        var myJSONObject = {
				        	...
				        	"title":{
				        		"text":data[i].procDefKey
				        	},
				        	"detail":{
				        		"text":"委托人：" + data[i].consigner + " | " + "受委托人：" + data[i].mandatary
				        	}
				        };
	...
	<header>
		<titlebar title="流程委托" iconhref="window.close();" ricon="res:image/contact/collapse.png" riconhref="http://luhai/mobile/delegate/todo/-1"/>
		<select id="filterProcDefKey" prompt="请选择流程模块" style="width:75%;">
			<option value="">全部</option>
			<c:forEach var="procIns" items="${my:getAllProcIns()}">
				<option value="${procIns.id}">${procIns.value}</option>
			</c:forEach>
		</select>
	...
	<body onload="initList();" backbind="window.close();" onl2rside="window.close();">
		<list id="delegateList" onscrollbottom="initList();">
			<item data-role="listheader">
				<hr/>
			</item>
			<item data-role="listitem" style="background-click-color:#e5e5e5">
				<br size="8"/>
				<font id="title" style="font-weight:bold;">流程模块</font>
				<br size="4"/>
				<font id="detail" style="color:#5f5f5f;font-size:16;">委托人、受委托人</font>
				<br size="8"/>
				<hr/>
			</item>
