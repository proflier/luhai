...
function $(x){
	...
	obj.provide = function(){
		...
		if(arguments.length<3){
			...
			var ajax = $a.go(arguments[0], arguments[1], arguments[2], "$a.provide", null, arguments[3], arguments[4]);
...
String.prototype.tjt = function(){//tjt=Tempate and JSON Transformations
	if(this==null) return "";
	...
	for(var i=0;i<json.length;i++){		
		var str = this.replace(/\$\{([^\}]*)\}/g, function(s, s1){
			...
			if(s1.indexOf("=")==-1) return json[i][s1]?json[i][s1]:""; 			
