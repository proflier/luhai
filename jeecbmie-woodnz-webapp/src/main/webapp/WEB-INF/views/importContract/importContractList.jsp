...
$(function(){   
	dg=$('#dg').datagrid({  
		method: "get",
	    url:'${ctx}/foreignTrade/importContract/json', 
	    fit : true,
		fitColumns : true,
		border : false,
		striped:true,
	    ...
	    columns:[[    
			...
			{field:'relativePerson',title:'业务员',sortable:true,width:15},
			{field:'signatureDate',title:'签署日期',sortable:true,width:20},
			{field:'supplier',title:'供应商',sortable:true,width:20},
			{field:'transportMode',title:'运输方式',sortable:true,width:20},
...
function add() {
	d=$("#dlg").dialog({   
	    title: '添加合同',    
	    href:'${ctx}/foreignTrade/importContract/create',
	    modal:true,
	    fit:true,
	    style:{borderWidth:0},
	    closable:false,
...
function del(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/foreignTrade/importContract/delete/"+row.id,
...
function upd(){
	...
	d=$("#dlg").dialog({   
	    ...
	    closable:false,
	    style:{borderWidth:0},
	    href:'${ctx}/foreignTrade/importContract/update/'+row.id,
	    modal:true,
	    buttons:[{
			text:'提交申请', iconCls:'icon-hamburg-up',
			handler:function(){
...
function detail(){
	...
	d=$("#dlg").dialog({   
	    ...
	    style:{borderWidth:0},
	    closable:false,
	    href:'${ctx}/foreignTrade/importContract/detail/'+row.id,
	    modal:true,
	    buttons:[{
			text:'关闭',iconCls:'icon-cancel',
			handler:function(){
...
//配置协议
function toAgencyAgreement(){
	//var row = dg.datagrid('getSelected');
	//var rowAgency = agency_dg.datagrid('getSelected');
	//if(rowIsNull(row)) return;
	d_agen=$("#dlgAgency").dialog({   
	    title: '配置协议',    
	    width: 580,    
	    height: 350,  
	    href:'${ctx}/foreignTrade/importContract/toAgencyConfig/',
	    modal:true,
	    buttons:[{
			text:'保存',iconCls:'icon-save',id:'button-save',
			handler:function(){
...
function apply(){
	...
	parent.$.messager.confirm('提示', '您确定要提交申请？', function(data){
		if (data){
			...
			$.ajax({
				type:'get',
				url:"${ctx}/foreignTrade/importContract/apply/"+row.id,
				success: function(data){
			    	successTip(data,dg);
					if(data=='success'){
...
function callBack(){
	...
	parent.$.messager.confirm('提示', '您确定要撤回申请？', function(data){
		if (data){
			...
			$.ajax({
				type:'get',
				url:"${ctx}/foreignTrade/importContract/callBack/"+row.id+"/"+row.processInstanceId,
				success: function(data){
			    	successTip(data,dg);
					if(data=='success'){
...
function applyAnd(action){
	...
	$.post('${ctx}/foreignTrade/importContract/' + action, jsonData, function(data){
