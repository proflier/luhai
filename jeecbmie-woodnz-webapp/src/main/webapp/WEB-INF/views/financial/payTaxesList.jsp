...
$(function(){
	dg=$('#dg').datagrid({
		...
		columns:[[
			...
			{field:'taxRate',title:'税率',sortable:true,width:20},
		    {field:'taxMoney',title:'税金',sortable:true,width:20,
		    ...
		    {field:'rowTaxDate',title:'划税日期',sortable:true,width:20},
...
function apply(){
	...
	parent.$.messager.confirm('提示', '您确定要撤回申请？', function(data){
		if (data){
			...
			$.ajax({
				type:'get',
				url:"${ctx}/financial/payTaxes/apply/" + row.id,
				success: function(data){
			    	successTip(data,dg);
					if(data=='success'){
...
function applyAnd(action){
	...
	$.post('${ctx}/financial/payTaxes/' + action, jsonData, function(data){
