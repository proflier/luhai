...
(function(window,$){
	var prepaidAccount = {
		dataGrid: {},
		init: function() {
			$("#search").click(prepaidAccount.getData);
			$("#reset").click(prepaidAccount.resetForm);
			$("#viewSelections").click(prepaidAccount.getSelections);
			dataGrid = $("#pdrTable").datagrid({
				method: "get",
			    url:'${ctx}/account/prepaidAccount/data', 
			    fit : true,
				fitColumns : true,
				border : false,
				striped:true,	
	...
	window.prepaidAccount = (window.prepaidAccount) ? window.prepaidAccount : prepaidAccount;
	$(function(){
		prepaidAccount.init();
	});
})(window,jQuery);
