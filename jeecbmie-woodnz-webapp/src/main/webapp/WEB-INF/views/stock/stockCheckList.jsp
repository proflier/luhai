...
$(function(){   
	dg=$('#dg').datagrid({  
	    ...
	    columns:[[    
			...
			{field:'applyPerson',title:'申请人',sortable:true,width:15},
...
function apply(){
	...
	parent.$.messager.confirm('提示', '您确定要提交申请？', function(data){
		if (data){
			...
			$.ajax({
				type:'get',
				url:"${ctx}/stock/enterpriseStockCheck/apply/"+row.id,
				success: function(data){
			    	successTip(data,dg);
					if(data=='success'){
...
function applyAnd(action){
	...
	$.post('${ctx}/stock/enterpriseStockCheck/' + action, jsonData, function(data){
