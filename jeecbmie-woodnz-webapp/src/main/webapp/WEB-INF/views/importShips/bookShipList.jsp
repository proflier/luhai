...
$(function(){
	dg=$('#dg').datagrid({
		method: "get",
		url:'${ctx}/importShips/bookShip/json',
		fit : true,
		width:100,
		fitColumns : true,
		border : false,
		...
		columns:[[
		    ...
		    {field:'shipName',title:'船名',sortable:true,width:25},
			{field:'shipYear',title:'船龄',sortable:true,width:20},
			{field:'shipNo',title:'船次 ',sortable:true,width:20},
			 {field:'signedTime',title:'签订时间',sortable:true,width:25},
...
function add() {
	d=$("#dlg").dialog({
		fit:true,
	    title: '添加订船合同',
	    href:'${ctx}/importShips/bookShip/create',
	    modal:true,
	    closable:false,
	    style:{borderWidth:0},
	    buttons:[{
...
function upd() {
	...
	d=$("#dlg").dialog({
		fit:true,
	    title: '修改订船合同',
	    href:'${ctx}/importShips/bookShip/update/' + row.id,
	    modal:true,
	    closable:false,
	    style:{borderWidth:0},
	    buttons:[{
...
function del(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if(data){
			$.ajax({
				type:'get',
				url:"${ctx}/importShips/bookShip/delete/" + row.id,
...
function detail(){
	...
	d=$("#dlg").dialog({
		fit:true,
	    title: '查看明细',
	    href:'${ctx}/importShips/bookShip/detail/' + row.id,
	    modal:true,
	    closable:false,
	    style:{borderWidth:0},
	    buttons:[{
...
//申请
function apply(){
	business_page.procStartUrl="${ctx}/importShips/bookShip/apply";
...
function callBack(){
	...
	parent.$.messager.confirm('提示', '您确定要撤回申请？', function(data){
		if (data){
			...
			$.ajax({
				type:'get',
				url:"${ctx}/importShips/bookShip/callBack/" + row.id + "/" + row.processInstanceId,
				success: function(data){
			    	successTip(data,dg);
					if(data=='success'){
